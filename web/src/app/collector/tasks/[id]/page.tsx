...
export default async function CollectorTaskPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const deepLink = `robomate://task/${id}`
            ...
            {existingSubmission ? (
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-[var(--foreground-secondary)]">
                    Submission happens on iOS only. Launch the robomate.mp4 app, capture the task, and your confirmation state will appear here automatically.
                  </p>
                </div>
